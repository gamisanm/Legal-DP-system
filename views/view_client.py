...
def display_profile(doc, config, cities):
    ...
    if st.button("✏️ Редактировать профиль"):
        st.session_state.editing_client_profile = True

    if "editing_client_profile" in st.session_state and st.session_state.editing_client_profile:
        st.divider()
        ...
        with st.form("client_edit_form"):
            ...
            with col1:
                if st.form_submit_button("❌ Отмена"):
                    st.session_state.editing_client_profile = False
                    st.rerun()
            with col2:
            ...
            if submitted:
                ...
                if empty_fields:
                    ...
                    try:
                        ...
                        st.success("🎉 Профиль водителя успешно обновлен!")
                        st.session_state.editing_client_profile = False
                        st.rerun()
                    except Exception as e:
                        st.error(f"❌ Ошибка при обновлении профиля: {str(e)}")

def view_client(config, selected_city, cities):
    session_prefix = "client_"
    ...
    if st.session_state[session_prefix + "view_mode"] == "list":
        data = get_paginated_data(config, selected_city, context="client_view")
        if not data:
            st.info("Нет водителей для отображения. Попробуйте изменить фильтры или добавить новые.")
        else:
            st.subheader("📋 Список водителей")
            with st.container():
                cols = st.columns(2)
                for idx, doc in enumerate(data):
                    with cols[idx % 2]:
                        display_card(doc, config)
                        if st.button("👀 Открыть профиль", key=f"client_view_profile_{str(doc['_id'])}"):
