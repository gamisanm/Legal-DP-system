...
def display_profile(doc, config, cities=None):
    ...
    if st.button("✏️ Редактировать профиль"):
        st.session_state.editing_autopark_profile = True

    if "editing_autopark_profile" in st.session_state and st.session_state.editing_autopark_profile:
        st.divider()
        st.subheader("📝 Редактирование профиля")

        with st.form("autopark_edit_form"):
            values = {}
            ...
            with col1:
                if st.form_submit_button("❌ Отмена"):
                    st.session_state.editing_autopark_profile = False
                    st.rerun()
            with col2:
            ...
            if submitted:
                ...
                if empty_fields:
                    ...
                    try:
                        ...
                        st.success("🎉 Профиль автопарка успешно обновлен!")
                        st.session_state.editing_autopark_profile = False
                        st.rerun()
                    except Exception as e:
                        st.error(f"❌ Ошибка при обновлении профиля: {str(e)}")

def view_autopark(config, selected_city, cities):
    session_prefix = "autopark_"
    ...
    if st.session_state[session_prefix + "view_mode"] == "list":
        data = get_paginated_data(config, selected_city, context="autopark_view")
        if not data:
            st.info("Нет автопарков для отображения. Попробуйте изменить фильтры или добавить новые.")
        else:
            st.subheader("📋 Список автопарков")
            with st.container():
                cols = st.columns(2)
                for idx, doc in enumerate(data):
                    with cols[idx % 2]:
                        display_card(doc, config)
                        if st.button("👀 Открыть профиль", key=f"autopark_view_profile_{str(doc['_id'])}"):
