...
def add_autopark(config, cities):
    st.subheader("➕ Добавление нового автопарка")
    
    with st.form("add_autopark_form", clear_on_submit=True):
        values = {}
        ...
        submitted = st.form_submit_button("✅ Добавить автопарк")
        ...
        if submitted:
            ...
            if empty_fields:
                ...
                try:
                    ...
                    st.success("🎉 Автопарк успешно добавлен!")
                    st.balloons()
                except Exception as e:
                    st.error(f"❌ Ошибка при добавлении автопарка: {str(e)}")